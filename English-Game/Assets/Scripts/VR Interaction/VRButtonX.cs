...
public class VRButtonX : MonoBehaviour
{
    [SerializeField] private UnityEvent whenButtonIsPressed;

    [Header("Variables")]
    [SerializeField, Range(-0.1f, 0f), Tooltip("Button Travel Distance is the distance the button travels inwards until it is stopped. This should always be negative.")] private float buttonTravelDistance;
    ...
    void Update()
    {
        if (holdForever == false)
        {
            ...
            {
                CodeLibrary.SetX(transform, StartPos.x);
            }
        }
    }
    ...
    {
        if (isActive == false && transform.position.x - StartPos.x < buttonTravelDistance)
        {
            CodeLibrary.SetX(transform, buttonTravelDistance + StartPos.x);
